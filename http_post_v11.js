...
var Ollama_url = "http://192.168.0.3:11434/api/"
...
var tools_flag = true
var thinking_window //思考を表示するウィンドウ
var option_log = "" //引数のログを保存する
...
var tools = [
	{
		...
		{
			"name":"exCMD",
			"description":"Windowsコマンドを実行する唯一の方法です。ディレクトリの確認(dir)や、ファイルの読み込み、確認(type)、API実行(curl)などはすべてこのツールを使用してコマンドを発行して行います。",
			"parameters":
			{
				"type":"object",
				"required":["command"],
	...
	/*
	{
		...
		{
			"name":"replaceText",
			"description":"ファイルの内容を置換する唯一の方法です。",
			"parameters":
			{
				"type":"object",
				"required":["FileName","TargetText","ReplaceText"],
				"properties":
				{
					"FileName":
					{
						"type":"string",
						"description":"編集するファイルを指定してください。"
					},
					"TargetText":
					{
						"type":"string",
						"description":"置換する対象の文章"
					},
					"ReplaceText":
					{
						"type":"string",
...
async function Egent_O()
{
	...
	for(var i = 0; i < process.argv.length; i++)
	{
		...
		else if(process.argv[i] === "-h") //help
		{
			...
			console.log("-no-tools\t:Do not use Tools.")
			console.log("-thinking\t:Show How AI to thinking(Open New Window.)")
			console.log("-voice\t:Use Irodoir-TTS to speach AI answer.(未実装)\n\n")
			process.exit()
		}
		else if(process.argv[i] === "-no-tools") //toolsをなしに。
		{
			option_log = option_log + "tools is disable." + "\n"
			//debug("tools is disable.")
			tools_flag = false
		}
		else if(process.argv[i] === "-thinking") //思考を表示
		{
			option_log = option_log + "Show Thinking Window." + "\n"
			//debug("Show Thinking Window.")
			thinking_flag = true
			try
			{
				thinking_window = child_process.spawn("cmd.exe",["node receiver.js"],{stdio:["pipe","pipe","pipe"],encode:"utf8"})
		...
		{
			option_log = option_log + "model changing now..." + "\n"
			//debug("model changeing now...")
		...
		{
			option_log = option_log + "Egent_O : Debug Mode." + "\n"
			//debug("Egent_O : Debug Mode.")
			debug_flag = true
		}
...
async function ask_ai(history)
{
	return new Promise((resolve,reject) => {
		...
		{
			debug("Egent_O - ask_ai : send_data:")
		...
		try
		{
			var request = http.request(Ollama_url + mode,option,res => {
				...
				{
					debug(`Egent_O - ask_ai : Status Code: ${res.statusCode}`)
				}
				...
				res.on("end",async () => {//データが止まったら
					try
					{
						...
						{
							thinking_window.stdin.write(return_thinking)
...
async function read_write_chat(json_data)
{
	return new Promise((resolve,reject) => {
		...
		{
			var debug_json = JSON.stringify(json_data)
			//debug("read_write_chat : " + err + `\ndata:${json_data}`)
			debug("read_write_chat : " + err + `\ndata:${debug_json}`)
		}
	})
}
...
function exCMD(command)
{
	...
	response_LINE("Running Command... : " + command)
	try
	{
			...
			var server = https.createServer(line_option,(req,res) => {
					if(req.method === 'POST')
					{
						...
						req.on('end',function()
						{
							...
							{
								debug(err)
								fs.appendFileSync(hacking_file,new Date() + line_data + "\n\n")//エラー原因を書き留める
								resolve()
							}
